...
class fusionLossFunc_improved(nn.Module):
    ...
    def forward(self, featureMaps, landmarks):
        ...
        X = np.clip(np.round((landmarks[:, :, 0] * (h - 1)).numpy()).astype("int"), 0, h-1)
        Y = np.clip(np.round((landmarks[:, :, 1] * (w - 1)).numpy()).astype("int"), 0, w-1)
        ...
        temloss = [
            [
                ...
                self.l1Loss(
                    featureMaps[imageId][
                        landmarkId + self.landmarkNum*1
                    ][indexs[imageId][landmarkId]],
                    self.offsetMapX_groundTruth[imageId][landmarkId][indexs[imageId][landmarkId]]
                ),
                self.l1Loss(
                    featureMaps[imageId][
                        landmarkId + self.landmarkNum*2
                    ][indexs[imageId][landmarkId]],
                    self.offsetMapY_groundTruth[imageId][landmarkId][indexs[imageId][landmarkId]]
                )
            ] if X[imageId][landmarkId] > 0 and Y[imageId][landmarkId] > 0 else [0,0,0]
            for imageId in range(self.imageNum)
            for landmarkId in range(self.landmarkNum)
        ]

        loss1 = (sum([
            sum(temloss[ind]) for ind in range(self.imageNum * self.landmarkNum)
